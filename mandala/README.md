...
- **Auto-draw** (default) — auto-draw motion settings, multi-line key colors (note maps to keys 1–9), spawn radius 2%–90% by pitch.
- **Drums** — same auto-draw motion, but General MIDI drum notes map to **drum families** with spawn rings (kick near center, cymbals on the outer arc). Colors use the multi-line palette: kick → key 1, snare → 2, tom → 3, hi-hat → 4, cymbal → 5, other → 6.
